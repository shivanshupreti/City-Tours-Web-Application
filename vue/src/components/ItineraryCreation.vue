...
<template>
  <div class="container">
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <div class="form-group">
        <label for="city">City:</label>
        <select id="city" v-model="itinerary.city" @change="handleCityChange" required>
          <option value="Paris">Paris</option>
          <option value="Rome">Rome</option>
          <option value="Kyoto">Kyoto</option>
          <option value="New York City">New York City</option>
      ...
      <div class="form-group">
        ...
        <select id="startingPoint" v-model="itinerary.startingPoint" required>
          <option v-for="point in startPoints" :key="point.startingPointId" :value="point.startingPointId">
            {{ point.startingPointName }}
          </option>
        </select>
      </div>
      <div class="form-group">
...
export default {
  ...
  methods: {
    ...
    async handleCancel() {
      ...
      this.itinerary.city = newCity;
      await this.fetchLandmarks();
      await this.getStartingPointByCity(this.itinerary.city); // Fetch starting points for the new city
      window.scrollTo(0, 0);
    },
    async handleCityChange() {
      await this.fetchLandmarks();
      await this.getStartingPointByCity(this.itinerary.city); // Fetch starting points when city changes
    },
    async getStartingPointByCity(city) {
      try {
        const response = await ItineraryService.getStartingPointByCity(city);
        this.startPoints = response.data;
      } catch (error) {
        console.error('Error getting starting points:', error);
        alert('There was an error getting the starting points. Please try again.');
      }
    }
  },
  created() {
    this.fetchLandmarks();
    this.getStartingPointByCity(this.itinerary.city);
  }
};
</script>
